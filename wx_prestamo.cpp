...
MyDialogPrestamo::MyDialogPrestamo( wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style ) : wxDialog( parent, id, title, pos, size, style )
{
	...
	// Columns
	m_grid1->SetColSize( 0, 47 );
	m_grid1->SetColSize( 1, 121 );
	m_grid1->SetColSize( 2, 129 );
	m_grid1->SetColSize( 3, 135 );
	m_grid1->SetColSize( 4, 205 );
	m_grid1->EnableDragColMove( false );
	m_grid1->EnableDragColSize( true );
	m_grid1->SetColLabelValue( 0, wxT("ID") );
	m_grid1->SetColLabelValue( 1, wxT("Nombre Libro") );
	m_grid1->SetColLabelValue( 2, wxT("Fecha Prestamo") );
	m_grid1->SetColLabelValue( 3, wxT("Fecha Devolucion") );
	m_grid1->SetColLabelValue( 4, wxT("Fecha Devuelto Alumno") );
	...
	bSizerListaPrestamosAnteriores->Add( m_grid1, 1, wxALL|wxEXPAND|wxALIGN_CENTER_HORIZONTAL, 5 );
